...
def build_lemma_doc_freq(conn, verbose=True):
    ...
    get_document_frequencies_batch() and by line_search's passage-query
    reduction; safe to regenerate at any time.

    HISTORY: added in #235, silently DROPPED by the qere/ketiv rewrite of this
    script, which is why the Hebrew and Coptic indexes shipped without the
    table and the Latin one went stale as the corpus grew. Restored 2026-08-29;
    if you rework this script, this function and its call at the end of
    build_index() must survive.
    """
