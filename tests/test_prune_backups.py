...
class TestTheRule:
    def test_the_newest_copy_is_kept_however_old_it_is(self, tmp_path):
        ...
        keep, drop = plan(groups)
        assert [r[2] for r in keep] == [only]
        assert drop == []

    def test_by_default_only_the_newest_survives(self, tmp_path):
        ...
        assert sorted(r[2] for r in drop) == sorted([second, old])

    def test_keep_two_holds_the_second_whatever_its_age(self, tmp_path):
        new = _write(tmp_path, 'x.db.bak-a', 1)
        second = _write(tmp_path, 'x.db.bak-b', 300)
        third = _write(tmp_path, 'x.db.bak-c', 400)
        keep, drop = plan(collect([str(tmp_path)]), keep=2)
        assert sorted(r[2] for r in keep) == sorted([new, second])
        assert [r[2] for r in drop] == [third]

    def test_keep_days_spares_a_further_copy_while_it_is_fresh(self, tmp_path):
        new = _write(tmp_path, 'y.db.bak-a', 1)
        fresh = _write(tmp_path, 'y.db.bak-b', 5)
        stale = _write(tmp_path, 'y.db.bak-c', 60)
        keep, drop = plan(collect([str(tmp_path)]), keep_days=30)
        assert sorted(r[2] for r in keep) == sorted([new, fresh])
