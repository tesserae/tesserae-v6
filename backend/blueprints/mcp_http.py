...
def _t_compare_texts(a):
    ...
    common = (
        ...
        "The channels are not all lexical: besides shared words, fusion matches on MEANING (semantic), "
        "GRAMMAR (syntax), synonyms (dictionary), and SOUND. A parallel found by meaning or grammar may "
        "share NO words, so it has nothing to bold -- present it on its own terms (quote both lines, say it "
        "is a meaning echo or a shared construction, not a word match) and never drop it for lacking shared "
        "words. Name the kind of similarity, so the reader sees the search reaches past vocabulary. "
        ...
        "never 'N+'. "
        "CITATIONS -- the FORM the characterization takes at each size: when an entry's corpus count is "
        "under 6, list EVERY occurrence inline in compact canonical form, e.g. 'Verg. Aen. 2.31; Stat. "
        "Theb. 12.531; Claud. Olybr. 84; Macr. Sat. 5.5.3 (quoting Vergil)' -- the reader is a scholar who "
        "wants to gaze over the actual places, and mark any occurrence that quotes an earlier line "
        "verbatim. From 6 up to ~40, characterize at the resolution the count allows: name the works when "
        "they are few ('twice in Vergil, four times in Statius, once each in Silius and Prudentius, plus "
        "one Renaissance treatise quoting Vergil'), the authors when the author list is short, the periods "
        "when it is long ('scattered from the Augustans through the Carolingians, thickest in late "
        "antiquity'). Above ~40 the count plus 'too common to signify' stands. This applies to rare-word "
        "entries on their corpus_count basis as well. "
        "When a rare-phrase/rare-word rarity metric and the line_search corpus check "
        ...
        "one row per phrase when several share a chart, or a single detailed row when the user asks about "
        "one phrase. line_search hits carry era, year, author, work, and locus -- draw from those. "
        "Keep the encoding dimensions SEPARATE, one legend entry per dimension, never a legend entry that "
        "mixes them: (1) COLOR answers WHERE an occurrence sits -- the source text, the target text, or "
        "elsewhere in the corpus -- and nothing else; (2) a HOLLOW marker answers HOW the phrase arrives "
        "there -- the occurrence quotes or repeats an earlier line verbatim rather than reusing the phrase "
        "independently -- and it composes with any color (a hollow dot is transmission of the phrase, not "
        "new life for it, which is what makes it worth marking in a genealogy); (3) an occurrence with no "
        "recorded date goes in a labeled 'undated' gutter at the edge of the axis, with no special marker, "
        "because its position already says everything. "
