...
class Matcher:
    ...
    def build_stoplist_manual(self, units, stoplist_size=10, language='la', match_type='lemma'):
        ...
        if language == 'la':
            ...
            try:
                from backend.arabic.stopwords import ARABIC_STOP_WORDS
                base_stops = set(list(ARABIC_STOP_WORDS)[:stoplist_size])
            except ImportError:
                base_stops = set()
        elif language == 'fa':
            try:
                from backend.persian.stopwords import PERSIAN_STOP_WORDS
                base_stops = set(list(PERSIAN_STOP_WORDS)[:stoplist_size])
            except ImportError:
                base_stops = set()
        elif language == 'he':
            try:
            ...
            try:
                from backend.urdu.stopwords import URDU_STOP_WORDS
                base_stops = set(list(URDU_STOP_WORDS)[:stoplist_size])
            except ImportError:
                base_stops = set()
        else:
